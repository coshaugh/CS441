...
class SignupHandler(BaseHandler):
	...
	def post(self):
		...
		if not user_data[0]:	# user_data is a tuple
			self.display_message('Unable to create user %s for email %s because of \
				duplicate keys %s' % (user_name, email, user_data[1]))
			return
		...
		# 	'user.name' : user_name
		# }
		# self.redirect('authenticated.html', params)
...
class LoginHandler(BaseHandler):
	...
	def _serve_page(self, failed=False):
		...
		self.render_template('login.html', params)	# passes username to pre-fil field
